...
func (r *Rand) Perlin(alpha, beta float32, n int) *Perlin {
	return newPerlinRandSource(alpha, beta, n, r.src)
}
...
func newPerlinRandSource(alpha, beta float32, n int, source rand.Source) *Perlin {
	var p Perlin
	var i int
	...
	for i = 0; i < B; i++ {
		p.p[i] = i
		p.g1[i] = float32((r.Int()%(B+B))-B) / B

		for j := 0; j < 2; j++ {
			p.g2[i][j] = float32((r.Int()%(B+B))-B) / B
		}

		normalize2(&p.g2[i])

		for j := 0; j < 3; j++ {
			p.g3[i][j] = float32((r.Int()%(B+B))-B) / B
		}
		normalize3(&p.g3[i])
	}
...
func normalize2(v *[2]float32) {
	s := sqrt(v[0]*v[0] + v[1]*v[1])
	v[0] = v[0] / s
	v[1] = v[1] / s
}

func normalize3(v *[3]float32) {
...
func at2(rx, ry float32, q [2]float32) float32 {
	return rx*q[0] + ry*q[1]
}

func at3(rx, ry, rz float32, q [3]float32) float32 {
	return rx*q[0] + ry*q[1] + rz*q[2]
}

func sCurve(t float32) float32 {
	return t * t * (3. - 2.*t)
}
...
func (p *Perlin) noise1(arg float32) float32 {
	var vec [1]float32
	vec[0] = arg
...
func (p *Perlin) noise2(vec [2]float32) float32 {
...
func (p *Perlin) noise3(vec [3]float32) float32 {
	t := vec[0] + N
	bx0 := int(t) & BM
	bx1 := (bx0 + 1) & BM
	rx0 := t - float32(int(t))
